...
class ReorgStakeTest(CryptosharesTestFramework):
    ...
    def check_money_supply(self, expected_shares):
        # verify that nodes have the expected SHARES supply
        shares_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(shares_supply, [DecimalAmt(expected_shares)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that SHARES supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check SHARES supply...")
