...
to the CRYPTOSHARES GitHub release page.
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cryptoshares-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cryptoshares-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cryptoshares-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cryptoshares-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cryptoshares-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cryptoshares-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/cryptoshares-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@cryptoshares.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cryptoshares-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cryptoshares-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cryptoshares-osx-*-build.assert
```
