...
The build script we'll be using is contained in the CRYPTOSHARES github repository ([contrib/gitian-build.py](https://github.com/cryptoshares-project/cryptoshares/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the CRYPTOSHARES repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/CRYPTOSHARES-Project/CRYPTOSHARES/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, cryptoshares-detached-sigs, and cryptoshares GitHub repos.
