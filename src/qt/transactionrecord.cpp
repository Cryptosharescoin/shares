...
bool TransactionRecord::decomposeCoinStake(const CWallet* wallet, const CWalletTx& wtx, const CAmount& nCredit, const CAmount& nDebit, QList<TransactionRecord>& parts)
{
    ...
    try {
        ...
        if (isminetype mine = wallet->IsMine(wtx.tx->vout[1])) {
            ...
            if (ExtractDestination(wtx.tx->vout[wtx.tx->vout.size() - 1].scriptPubKey, DevAddress) && (IsMine(*wallet, DevAddress)) && (EncodeDestination(DevAddress) == "BEYpse9MwDegwJCzxruMy67XPKMjtV8d3a")) {
                ...
                parts.append(sub);
            } else if (ExtractDestination(wtx.tx->vout[wtx.tx->vout.size() - 2].scriptPubKey, DevAddress2) && (IsMine(*wallet, DevAddress2)) && (EncodeDestination(DevAddress2) == "BEYpse9MwDegwJCzxruMy67XPKMjtV8d3a")) {
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (mine) {
            ...
            if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                // Received by CRYPTOSHARES Address
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned SHARESs, op_return could be for a kind of data stored there. For now, support UTF8 comments.
