...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeCryptosharesAmount") || reset)
        gArgs.SoftSetArg("-anonymizecryptosharesamount", settings.value("nAnonymizeCryptosharesAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cryptoshares.cpp during shotdown to also remove the window positions
