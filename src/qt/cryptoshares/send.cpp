...
#include "qt/cryptoshares/forms/ui_send.h"
    ...
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-shares");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(sharesAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
