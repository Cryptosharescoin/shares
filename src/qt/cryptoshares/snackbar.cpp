...
#include "qt/cryptoshares/forms/ui_snackbar.h"
#include "qt/cryptoshares/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(CRYPTOSHARESGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &CRYPTOSHARESGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
