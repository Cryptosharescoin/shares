...
CzSHARESWallet::CzSHARESWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zSHARES seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSHARESSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSHARESSeed_deprecated()) {
                    LogPrintf("%s: Updated zSHARES seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zSHARES seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zSHARES wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSHARESWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSHARESCount(nCountLastUsed);
    else if (!walletdb.ReadZSHARESCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSHARESWallet::Lock()
{
    seedMaster.SetNull();
}

void CzSHARESWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSHARESWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSHARES(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSHARESWallet::LoadMintPoolFromDB()
{
...
void CzSHARESWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSHARESWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSHARESWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zsharesTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSHARESWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSHARES(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zSHARESTracker which also adds to database
    wallet->zsharesTracker->Add(dMint, true);
...
void CzSHARESWallet::SeedToZSHARES(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzSHARESWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSHARESWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(wallet->strWalletFile);
    walletdb.WriteZSHARESCount(nCountLastUsed);
...
void CzSHARESWallet::GenerateDeterministicZSHARES(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSHARESWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSHARESWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSHARESWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
