...
bool CLegacyZSharesStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZSHARESStake from non zc-spend", __func__);
    ...
    *this = CLegacyZSharesStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zSHARES stake origin", __func__);

    // All good
    return true;
}

CLegacyZSharesStake::CLegacyZSharesStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZSharesStake::GetIndexFrom()
{
...
CAmount CLegacyZSharesStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZSharesStake::GetUniqueness() const
{
...
bool CLegacyZSharesStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zSHARES stake block: height %d outside range", __func__, nHeight);
