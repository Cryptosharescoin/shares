...
#include "zshares/zshareswallet.h"


CzSHARESTracker::CzSHARESTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzSHARESTracker::~CzSHARESTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSHARESTracker::Init()
{
...
bool CzSHARESTracker::Archive(CMintMeta& meta)
{
...
bool CzSHARESTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSHARESTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSHARESTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSHARESTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSHARESTracker::GetSerialHashes()
{
...
CAmount CzSHARESTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSHARESTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSHARESTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSHARESTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSHARESTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSHARESTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSHARESTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSHARESTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSHARESTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSHARESTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzSHARESTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSHARESWallet* zSHARESWallet)
{
    bool iszSHARESWalletInitialized = (nullptr != zSHARESWallet);
    CMintMeta meta;
    ...
    if (!iszSHARESWalletInitialized)
        zSHARESWallet = new CzSHARESWallet(wallet);
    meta.isSeedCorrect = zSHARESWallet->CheckSeed(dMint);
    if (!iszSHARESWalletInitialized)
        delete zSHARESWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSHARESTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSHARESTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSHARESTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSHARESTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSHARESTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSHARESTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzshares from DB\n", __func__, listDeterministicDB.size());
    }
