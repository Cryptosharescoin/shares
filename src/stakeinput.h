...
#ifndef CRYPTOSHARES_STAKEINPUT_H
#define CRYPTOSHARES_STAKEINPUT_H
...
class CSharesStake : public CStakeInput
{
private:
...
public:
    CSharesStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CSharesStake* NewSharesStake(const CTxIn& txin, int nHeight, uint32_t nTime);
...
#endif //CRYPTOSHARES_STAKEINPUT_H
