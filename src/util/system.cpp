...
const char* const CRYPTOSHARES_CONF_FILENAME = "shares.conf";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cryptosharesd -foo=bar
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SHARES";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SHARESParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "SHARESParams";
#else
    // Unix
    return pathRet / ".shares-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/cryptoshares/sapling-spend.params";
        sapling_output = "/usr/share/cryptoshares/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/cryptoshares/sapling-spend.params";
            sapling_output = "/usr/local/share/cryptoshares/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", CRYPTOSHARES_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()) {
        // Create empty shares.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(confPath), "a");
        if (configFile != NULL) {
            std::string strHeader = "rpcuser=sharesuser\n"
                                    "rpcpassword=sharespassword\n"
                                    "rpcport=23191\n"
                                    "port=23190\n"
                                    "server=1\n"
                                    "daemon=1\n"
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", CRYPTOSHARES_PID_FILENAME));
