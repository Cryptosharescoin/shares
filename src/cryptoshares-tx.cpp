...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " cryptoshares-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  cryptoshares-tx [options] <hex-tx> [commands]  Update hex-encoded cryptoshares transaction\n" +
                               "or:     cryptoshares-tx [options] -create [commands]   Create hex-encoded cryptoshares transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded cryptoshares transaction
