...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != Params().GetConsensus().GetMNCollateral(chainActive.Height())) {
        strError = strprintf("Invalid collateral tx value, must be %s SHARES", FormatMoney(Params().GetConsensus().GetMNCollateral(chainActive.Height())));
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-cryptoshares-address
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CSharesStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
