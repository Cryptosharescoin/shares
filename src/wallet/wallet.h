...
#include "zshares/zsharestracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_CRYPTOSHARES = 61000, // inherited version..
...
// Possible states for zSHARES send
enum ZerocoinSpendStatus {
    ZSHARES_SPEND_OKAY = 0,                            // No error
    ZSHARES_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSHARES_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSHARES_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSHARES_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSHARES_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSHARES_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSHARES_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSHARES_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSHARES_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZSHARES_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSHARES_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSHARES_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSHARES_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSHARES_SPENT_USED_ZSHARES = 14,                      // Coin has already been spend
    ZSHARES_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZSHARES_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
private:
    ...
    // Zerocoin wallet
    CzSHARESWallet* zwallet{nullptr};
...
public:
    ...
    void doZSharesRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZSHARES);
    ...
    bool CreateZSHARESOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zSHARES wallet
    std::unique_ptr<CzSHARESTracker> zsharesTracker{nullptr};
    void setZWallet(CzSHARESWallet* zwallet);
    CzSHARESWallet* getZWallet();
