...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k SHARES
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Cryptoshares Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        ...
        if (devrewardvalue > 0) {
            ...
            for (const CTxOut& txout : block.vtx[1]->vout) {
                ...
                if ((txout.nValue == devrewardvalue) == true) {
                   if ((found = EncodeDestination(Dest) == Params().getDevRewardAddress(nHeight)))
                        break;
                }
            }
            if (!found)
