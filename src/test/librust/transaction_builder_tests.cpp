...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-SHARES in, 0.4 shielded-SHARES out, 0.1 t-SHARES fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling)
{
    ...
    // --- 0.4 shielded-SHARES in, 0.299 shielded-SHARES out, 0.1 shielded-SHARES fee, 0.001 shielded-SHARES change (added to fee)
    ...
    // --- Now try with 1 shielded-SHARES in, 0.5 shielded-SHARES out, 0.1 shielded-SHARES fee, 0.4 shielded-SHARES change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 SHARES note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-SHARES out, 0.1 t-SHARES fee
    auto builder = TransactionBuilder(consensusParams);
    ...
    // 0.5 t-SHARES out, 0.1 t-SHARES fee
    ...
    // 0.5 t-SHARES out, 0.1 t-SHARES fee, 0.59999 shielded-SHARES in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
