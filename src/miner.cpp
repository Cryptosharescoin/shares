...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("CRYPTOSHARESMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("CRYPTOSHARESMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("CRYPTOSHARESMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running CRYPTOSHARESMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("CRYPTOSHARESMiner exception");
    } catch (...) {
        LogPrintf("CRYPTOSHARESMiner exception");
    }

    LogPrintf("CRYPTOSHARESMiner exiting\n");
}
